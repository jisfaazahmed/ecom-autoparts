...
/**
 * GET /api/shops - paginated list (same as /all for superadmin)
 */
...
/**
 * GET /api/shops/all - paginated list of all shops (alias)
 */
exports.getAllShops = exports.getShops;
...
/**
 * GET /api/shops/:id - single shop by id (ownerId / user id)
 */
...
/**
 * PUT /api/shops/:id/status - update vendor status (superadmin)
...
/**
 * PUT /api/shops/:id/commission - update commission rate (superadmin)
 ...
 * PUT /api/shops/my - update my shop
 */
exports.updateShop = async (req, res) => {
  try {
    const id = req.params.id === 'my' ? req.user.id : req.params.id;
    const isOwn = id === req.user.id;
    ...
    const allowed = ['shopName', 'shopDescription', 'phone', 'address', 'businessRegistration', 'logoUrl'];
    const updates = {};
    const body = req.body || {};
    allowed.forEach((key) => {
